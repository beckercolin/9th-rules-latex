...
\base{\portee{18}} \newline \amel{\portee{36}} \newline Malédiction &
Dure un tour &
\newrule{La cible ne peut pas se déplacer de plus de \distance{10} durant la phase des Autres Mouvements. Toutes les unités ennemies dans un rayon de \distance{6} de la cible ont -1 en Capacité de Tir.}
\tabularnewline
\hline
2 & Choc Foudroyant &
\base{8+}\newline \amel{11+} &
...
\tabularnewline
\hline
5 & \newrule{Remparts Incandescents} &
\base{10+}\newline \amel{13+} &
\base{\portee{24}} \newline \amel{\portee{48}} \newline Malédiction &
Reste en jeu &
La cible subit 1D6 touches de Force 4 avec la règle \emph{Attaques Enflammées}. A la fin de chaque Phase de jeu, chaque figurine de l'unité subit une touche de Force 4 avec la règle \emph{Attaques Enflammées} si l'unité a effectué une ou plusieurs des actions suivantes durant la phase: Charge, Charge Irrésistible, \newrule{Charge Ratée}, Fuite, Marche Forcée, Mouvement Simple, Pivot, Poursuite, Reformation ou Reformation de Combat. Une même unité ne peut subir ces touches qu'une fois par Phase.
\tabularnewline
\hline
...
\portee{12} \newline Amélioration \newline \amel{Aura} &
Dure un tour &
La cible gagne +1 Attaque, la règle \newrule{\emph{Attaques Divines}} et double son Mouvement jusqu'à un maximum de 10.
\tabularnewline
\hline
6 & Bannissement Divin &
\base{10+}\newline \amel{13+} &
\portee{24} \newline Malédiction \newline Projectile \newline Dégâts &
Immédiat &
...
\base{\portee{24}} \newline \amel{\portee{48}} \newline \amelbis{\portee{48}} \newline Malédiction \newline Projectile \newline Dégâts &
Immédiat &
La cible subit \base{1D6}/\amel{1D6}/\amelbis{1D6+1} touches de Force 1D6/\amel{1D6}/\amelbis{1D6+1} avec la règle \emph{Flammes de l'Enfer}.
\tabularnewline
\hline
1 & Feu Carmin &
\base{6+}\newline \amel{11+} &
...
\tabularnewline
\hline
2 & Bénédiction de la Mère-Araignée! &
\base{6+}\newline \amel{9+} &
\base{\portee{12}} \newline \amel{\portee{24}} \newline Amélioration &
Dure un tour &
...
\newline Ignorez la prochaine blessure non sauvegardée (après avoir calculé les éventuelles \emph{Blessures Multiples}), et retirez le marqueur \emph{Bouclier}. \newrule{Si la cible subit plusieurs blessures simultanées, son propriétaire choisit quelle blessure est ignorée.
\newline S'il arrive qu'une unité ait plusieurs marqueurs \emph{Bouclier} à la fois, retirez-les tous immédiatement, sauf un.}
\tabularnewline
\hline
0 & \newrule{Traits de Lumière} &
\base{9+} \newline \amel{12+} &
\base{\portee{24}} \newline \amel{\portee{18}} \newline Malédiction \newline Projectile \newline Dégâts &
Immédiat &
...
\portee{24} \newline Focalisé \newline Direct \newline Malédiction \newline Dégâts &
Immédiat &
La cible subit une touche avec la règle \emph{Fusion du Métal}. Si la cible porte un ou plusieurs objets magiques, l'un d'entre eux, au hasard, est détruit. Seuls les objets qui ne sont pas à \emph{Usage Unique} peuvent être ainsi détruits (ignorez les autres durant le tirage au sort).
\tabularnewline
\hline
6 & Cataclysme &
\base{14+} \newline \amel{18+} &
...
\newline Si cela cause une blessure non sauvegardée, le lanceur peut Récupérer 1 PV. Aucune figurine ne peut gagner plus d'un PV par Phase de Magie avec ce sort.
\tabularnewline
\hline
0 & Furie de Moraec &
\base{8+} \newline \amel{10+} &
...
&
Immédiat &
La cible subit 1D6 touches de Force 5 avec la règle \emph{Attaques Foudroyantes}. \newrule{Si le nombre de touches obtenu est 6, le lanceur subit une touche de Force 5, en plus de celles infligées à la cible.}
\tabularnewline
\hline
1 & \newrule{Nourrissons-les...} &
\base{6+} \newline \amel{10+} &
\portee{24} \newline Amélioration &
Dure un tour &
\newrule{L'unité ciblée gagne +1 pour blesser au corps à corps. \amel{Elle a aussi la règle \emph{Coup Fatal}.}}
\tabularnewline
\hline
2 & \newrule{Souiller le Sol} &
8+ &
\portee{18} \newline Marqueur &
Spécial &
Dure jusqu'au début du prochain tour du lanceur. Placez le gabarit de \newrule{\distance{3}} centré sur le marqueur et à minimum \distance{1} de toute unité. Si une unité touche le gabarit lors d'un déplacement sans \emph{Vol}, toutes ses figurines traitent n'importe quel terrain (y compris les Terrains Découverts) comme Terrain Dangereux (2).
\tabularnewline
\hline
3 & La Faim &
\base{8+} \newline \amel{11+} &
...
\tabularnewline
\hline
4 & \newrule{Appel de la Tempête} &
\base{9+} \newline \amel{10+} &
Spécial &
Dure un tour &
Toutes les unités ennemies subissent un malus de -1 en CT.
\newline \amel{Impossible d'effectuer de \emph{Vol} sur tout le champ de bataille.}
\tabularnewline
\hline
5 & Rupture Sismique &
\base{9+} \newline \amel{12+} &
...
\tabularnewline
\hline
6 & \newrule{Pour Qui Sonne le Glas} &
\base{11+} \newline \amel{14+} &
\base{\portee{12}} \newline \amel{\portee{24}} \newline Direct &
Immédiat &
Toutes les figurines de l'unité subissent une touche avec la règle \emph{Attaques Toxiques}.
\newline Si ce sort est lancé sur une unité engagée en corps à corps, toutes les figurines engagées dans le combat subissent une touche avec la règle \emph{Attaques Toxiques}.
\tabularnewline
\closetable
