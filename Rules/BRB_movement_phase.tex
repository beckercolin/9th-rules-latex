...
Déplacez chaque unité en fuite droit devant de \unit{2D6}{\pouce}. Si ce mouvement devait faire terminer l'unité à moins de \unit{1}{\pouce} d'une autre unité ou d'un \emph{Terrain Infranchissable}, augmentez la distance de fuite du minimum nécessaire pour éviter cette infraction. \nouveau{Si des figurines d'une unité en fuite traversent des figurines ennemies ou un \emph{Terrain Infranchissable}, elles doivent faire un test de \emph{Terrain Dangereux} qui échoue sur un jet de '1' à '3'}. Si ce mouvement de fuite amène l'unité en contact d'un bord de table, voire le dépasse, l'unité est détruite au moment du contact, provoquant éventuellement des tests de \emph{Panique} aux unités aux alentours. Notez que ce mouvement de fuite est souvent précédé d'un pivot, qui suit les mêmes règles que le mouvement de fuite. Les mouvements de fuite ignorent tout obstacle.
...
Quand une unité fait une \emph{Roue}, tournez l'unité autour d'un de ses coins avant. La distance parcourue par l'unité correspond à celle parcourue par la figurine du coin avant opposé. \nouveau{En plus de ceci, quand un déplacement comprend une ou plusieurs \emph{Roues}, aucune figurine ne doit bouger de plus de sa valeur de Mouvement (ou deux fois pendant une \emph{Marche Forcée} réussie) depuis sa position initiale (voir figure \ref{figure/roue}). Si le déplacement fait partie d'une \emph{Reformation Rapide}, la distance est mesurée depuis la position après la \emph{Reformation}}.
